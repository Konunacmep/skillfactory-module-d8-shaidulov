...
from .signals import priority_counter
...
def index(request):
    ...
    # 3rd version
    prio_count = cache.get('prio_count')
    if not prio_count:
        prio_count = priority_counter()
    ...
    return render(request, "tasks/index.html", context={'counts': counts, 'prio_counts': prio_count})
...
class DataNowView(TemplateView):
    template_name = 'tasks/cached_now.html'
